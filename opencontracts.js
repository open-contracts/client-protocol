 ...
 async function enclaveSession(opencontracts, f) {
     ...
     ws.onmessage = async function (event) {
         ...
         if (data['fname'] == "print") {
             ...
             /**
              * Note: passing data["key"] passes a reference to data["key"] which may be mutated.
              * This can become problematic in concurrent systems.
              * Instead, I recommend creating a closure with a clone of the data object.
              * 
              * That being said, if there are things that should remain as references,
              * then reference them using data instead of _data.
              */
             const _data = Object.assign({}, data);
             await f.submitHandler(async function() {
                 return await requestHubTransaction(opencontracts,
                      _data['nonce'], 
                      _data['calldata'], 
                      _data['oracleSignature'],
                     _data['oracleProvider'], 
